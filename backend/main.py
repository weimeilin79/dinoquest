...
async def get_leaderboard(authorization: str = Header(None)):
    """
    Fixed: Use server-side ordering and limit to prevent OOM.
    Removed artificial memory hog 'replay_frames'.
    """
    ...
    # Fetch top 100 scores ordered by score descending
    ...
    for doc in docs:
        data = doc.to_dict()
        data["id"] = doc.id
        # Remove massive string allocation that was causing OOM
        scores.append(data)
